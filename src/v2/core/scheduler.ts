...
export class Scheduler {
  ...
  run(context: TickContext): void {
    const ordered = [...this.processes].sort((left, right) => PRIORITY_ORDER[left.priority] - PRIORITY_ORDER[right.priority]);
    ...
    for (const definition of ordered) {
      ...
      if (definition.maxCpu !== undefined && Game.cpu.getUsed() >= definition.maxCpu) {
        stats.skipped += 1;
        continue;
      }
